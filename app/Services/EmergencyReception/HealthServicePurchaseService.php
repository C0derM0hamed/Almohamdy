...
final class HealthServicePurchaseService
{
    public function list(array $filters): mixed
    {
        ...
        if ($filters['from']) $query->where('health_service_purchase_form.date', '>=', strtotime($filters['from']));
        if ($filters['to']) $query->where('health_service_purchase_form.date', '<=', strtotime($filters['to'].' 23:59:59'));
        if ($filters['status'] !== '') $query->where('health_service_purchase_form.verified', (int) $filters['status']);
        if ($filters['search']) $query->where(fn ($q) => $q->where('health_service_purchase_form.id_number', 'like', "%{$filters['search']}%")->orWhere('health_service_purchase_form.member_id_number', 'like', "%{$filters['search']}%"));
    ...
    public function downloadAttachment(int $id, int $attachment): mixed
    {
        ...
        if (Storage::disk('local')->exists($path)) return Storage::disk('local')->download($path, $name);
    ...
    public function publicRecord(string $token): ?object
    {
        ...
        if (count($parts) !== 3 || !ctype_digit($parts[1]) || !ctype_digit($parts[2])) return null;
    ...
    public function nationalities(): iterable { return DB::table('nationality')->where('publish', 1)->orderBy('name_ar')->get(); }
    private function scoped(): Builder { return DB::table('health_service_purchase_form')->where('health_service_purchase_form.branch_id', 1)->where('health_service_purchase_form.companies_groups_id', $this->companyId()); }
