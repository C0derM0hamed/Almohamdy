...
class AdmissionCalculatorController extends Controller
{
    ...
    public function pdf(string $type, int $id): mixed { $record = $this->service->find($type, $id); abort_if($record === null, 404); return app(ArabicPdfService::class)->loadView('admission-calculator.pdf', ['record' => $record, 'type' => $type])->setPaper('a4')->download('admission-calculator-'.$id.'.pdf'); }
}
