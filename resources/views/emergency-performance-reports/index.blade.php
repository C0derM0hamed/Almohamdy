...
<div class="container-fluid py-3" dir="rtl">
    ...
    <form method="get" class="card border-0 shadow-sm mb-4">
        <div class="card-body row g-3 align-items-end">
            ...
            <div class="col-md-2"><label class="form-label" for="period">{{ __('emergency_reports.period') }}</label><select class="form-select" id="period" name="period"><option value="all">{{ __('emergency_reports.all') }}</option>@foreach ($options['periods'] as $period)<option value="{{ $period->id }}" @selected($filters['period'] === (string) $period->id)>{{ app()->getLocale() === 'ar' ? $period->name_ar : $period->name_en }}</option>@endforeach</select></div>
