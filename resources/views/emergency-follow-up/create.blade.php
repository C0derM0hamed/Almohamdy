        ...
        <div class="card border-0 shadow-sm"><div class="card-body p-4">
            ...
            <form method="post" action="{{ route('modules.emergency-follow-up.store') }}" novalidate>@csrf
                <div class="row g-3">
                    ...
                    <div class="col-md-6"><label class="form-label" for="notice">{{ __('emergency_follow_up.notice') }}</label><select class="form-select @error('notice') is-invalid @enderror" id="notice" name="notice" required><option value="">—</option>@foreach ($noticeTypes as $type)<option value="{{ $type->id }}" @selected(old('notice') == $type->id)>{{ app()->getLocale() === 'ar' ? $type->name_ar : $type->name_en }}</option>@endforeach</select>@error('notice')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
