...
<div class="hm-module-page" data-module="training-detail">
 ...
 <div class="d-flex justify-content-between align-items-center mb-4"><div><h1 class="h3">{{ __('training.details') }} #{{ $training->id }}</h1><span class="badge bg-primary">{{ $training->currentStatus?->name_ar }}</span></div><a class="btn btn-outline-secondary" href="{{ route($routes['index']) }}">{{ __('training.back') }}</a></div>
 ...
 <div class="col-lg-5"><div class="card"><div class="card-header"><h2 class="h5 mb-0">{{ __('training.update_status') }}</h2></div><div class="card-body"><form method="POST" action="{{ route($routes['status'], $training->id) }}">@csrf
 ...
 <div class="card mt-4"><div class="card-header"><h2 class="h5 mb-0">{{ __('training.documents') }}</h2></div><div class="card-body d-flex flex-wrap gap-2">
  <a class="btn btn-outline-dark" href="{{ route($routes['document'], [$training->id, 'plan']) }}">{{ __('training.plan_pdf') }}</a>
  @if($training->hasSignedPdf())<a class="btn btn-outline-dark" href="{{ route($routes['signed_pdf'], $training->id) }}">{{ __('training.signed_pdf') }}</a>@endif
  @foreach([3=>'coordinator-passed',4=>'coordinator-failed',6=>'manager-passed',7=>'manager-failed'] as $status=>$document)@if($timeline->contains('status_id',$status))<a class="btn btn-outline-dark" href="{{ route($routes['document'], [$training->id,$document]) }}">{{ __('training.document_'.$document) }}</a>@endif @endforeach
  <a class="btn btn-outline-primary" href="{{ route($routes['timeline'], $training->id) }}">{{ __('training.timeline') }}</a>
 </div></div>
</div>
@push('scripts')<script>
(function () {
  ...
  document.getElementById('managementStatus')?.addEventListener('change', function () {
    document.getElementById('managementAck').hidden = this.value !== String(ackStatusId);
    document.getElementById('managementReason').hidden = !reasonStatusIds.includes(Number(this.value));
  });
})();
</script>@endpush
@endsection
