...
<div><small>{{ __('gov_accounts.fields.employee') }}</small><strong>{{ $account->employee?->displayName() }}</strong></div><div><small>{{ __('gov_accounts.fields.authority') }}</small><strong>{{ $account->authority?->localizedName() }}</strong></div><div><small>{{ __('gov_accounts.fields.service') }}</small><strong>{{ $account->service?->localizedName() }}</strong></div><div><small>{{ __('gov_accounts.fields.role') }}</small><strong>{{ $account->role?->localizedName() }}</strong></div><div><small>{{ __('gov_accounts.fields.reference_no') }}</small><strong>{{ $account->reference_no ?: '—' }}</strong></div><div><small>{{ __('gov_accounts.fields.login_url') }}</small>@if($account->login_url)<a href="{{ $account->login_url }}" rel="noopener noreferrer" target="_blank">{{ $account->login_url }}</a>@else<strong>—</strong>@endif</div>
</div></section>
