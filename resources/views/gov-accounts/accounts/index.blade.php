...
<section class="lic-panel"><div class="lic-table-wrap"><table class="lic-table"><thead><tr><th>{{ __('gov_accounts.fields.employee') }}</th><th>{{ __('gov_accounts.fields.branch') }}</th><th>{{ __('gov_accounts.fields.department_unit') }}</th><th>{{ __('gov_accounts.fields.authority') }}</th><th>{{ __('gov_accounts.fields.service') }}</th><th>{{ __('gov_accounts.fields.username') }}</th><th>{{ __('gov_accounts.fields.status') }}</th></tr></thead><tbody>
@forelse($accounts as $account)<tr><td><a href="{{ route('modules.gov-accounts.accounts.show',$account) }}">{{ $account->employee?->displayName() }}</a></td><td>{{ $account->hospitalBranch?->localizedName() ?? '—' }}</td><td>{{ $account->sourceRequest?->department?->hierarchyLabel() ?? '—' }}</td><td>{{ $account->authority?->localizedName() }}</td><td>{{ $account->service?->localizedName() }}</td><td>{{ $account->username }}</td><td>{{ __('gov_accounts.account_statuses.'.$account->status) }}</td></tr>@empty<tr><td colspan="7" class="lic-empty">{{ __('gov_accounts.accounts.none') }}</td></tr>@endforelse
