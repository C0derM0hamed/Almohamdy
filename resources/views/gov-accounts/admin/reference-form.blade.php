    ...
    <section class="lic-panel"><form method="POST" action="{{ $editing ? route('modules.gov-accounts.admin.'.$reference.'.update', $item) : route('modules.gov-accounts.admin.'.$reference.'.store') }}">
        ...
        <div class="lic-form-grid">
            ...
            @if($reference === 'department-heads')
                <div class="lic-field"><label for="department_id">{{ __('gov_accounts.fields.department_unit') }}</label><select id="department_id" name="department_id" class="form-select" required><option value="">—</option>@foreach($departments as $department)<option value="{{ $department->id }}" @selected((int)old('department_id', $item?->department_id) === (int)$department->id)>{{ $department->hierarchyLabel() }}</option>@endforeach</select>@error('department_id')<div class="text-danger">{{ $message }}</div>@enderror</div>
