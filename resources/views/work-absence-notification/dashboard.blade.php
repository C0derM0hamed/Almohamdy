...
@section('content')
    <div class="hm-fm hm-wan hm-wan--figma {{ $isRtl ? 'hm-wan--rtl' : '' }}">
        @include('layouts.partials.figma-module-header', [
            'compact' => true,
            'title' => __('work_absence_notification.dashboard'),
            'crumbs' => [
                ['label' => __('dashboard.modules')],
                ['label' => __('dashboard.nav.employee_services')],
                ['label' => __('work_absence_notification.dashboard')],
            ],
        ])

        <header class="wan-head">
            <span class="wan-head__icon" aria-hidden="true"><i class="bi bi-person-x"></i></span>
            <div>
        ...
        <section class="wan-recipients" aria-labelledby="wanRecipientsTitle">
            <h2 id="wanRecipientsTitle">{{ __('work_absence_notification.recipients.dashboard_section') }}</h2>
            <div class="wan-recipient-grid">
                @foreach ($recipientCards as $card)
                    <article class="wan-mini wan-mini--{{ $card['tone'] }}">
        ...
        @if ($hasDistributionCharts)
            <section class="wan-chart-grid wan-chart-grid--distribution {{ ! $hasTypeChart || ! $hasWorkflowChart ? 'wan-chart-grid--single' : '' }}">
                @if ($hasTypeChart)
                    <article class="wan-panel wan-panel--wide">
                        <div class="wan-panel__head">
                            <h2>{{ __('work_absence_notification.charts.type_distribution') }}</h2>
                        </div>
                        <div class="wan-chart-wrap wan-chart-wrap--bar">
        ...
        @if (count($reportPanels) > 0)
            <section class="wan-report-grid">
                @foreach ($reportPanels as $panel)
                    @include('work-absence-notification.partials.report-breakdown', array_merge($panel, ['chartIndex' => $loop->index]))
                @endforeach
            </section>
        @endif
    </div>

    @if ($hasCharts || count($reportPanels) > 0)
        @push('scripts')
            ...
            <script>
                ...
                window.hmWanReportCharts = @json($reportPanels);
            </script>
