...
return new class extends Migration
{
    ...
    {
        foreach (['escape_report_form_attachments', 'incident_report_form_attachments', 'health_service_purchase_form_attachments'] as $table) {
            Schema::dropIfExists($table);
        }
        if (Schema::hasTable('receive_unidentified_case') && Schema::hasColumn('receive_unidentified_case', 'number')) {
            Schema::table('receive_unidentified_case', fn (Blueprint $table) => $table->dropColumn('number'));
        }
    }
};
